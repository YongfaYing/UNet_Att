...
class trainset(Dataset):
    def __init__(self,name_list,coordinate_list,noise_img, target_img):
    ...
    def __getitem__(self, index):
        ...
        input = self.noise_img[init_h:end_h, init_w:end_w]
        target = self.target_img[init_h:end_h, init_w:end_w]
...
class trainset_fca(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #print(input.shape)
        fft=np.fft.fftn(input,axes=(1,2))  #傅里叶变换
        absfft=pow(abs(fft)+1e-8,0.1)    #运算后，复数变成浮点数
        fca=np.fft.fftshift(absfft, axes=(1,2)).astype('float32')
        #print(fca.shape)
        input=torch.from_numpy(np.expand_dims(input, 0))
        target=torch.from_numpy(np.expand_dims(target, 0))
        fca=torch.from_numpy(np.expand_dims(fca, 0))
        #target = self.target[index]
        return input, target, fca
...
def singlebatch_test_save(single_coordinate,output_image,raw_image):     #test整合图像
    ...
    bbbb = raw_image[patch_start_h:patch_end_h, patch_start_w:patch_end_w]
    return aaaa,bbbb,stack_start_w,stack_end_w,stack_start_h,stack_end_h
    

def multibatch_test_save(single_coordinate,id,output_image,raw_image):     #test整合图像
    ...
    bbbb = raw_image_id[patch_start_h:patch_end_h, patch_start_w:patch_end_w]
...
def train_preprocess_lessMemoryMulStacks(args):   #训练用图像预处理
    img_h = args.img_h
    img_w = args.img_w
    gap_h = args.gap_h
    gap_w = args.gap_w
    im_folder = args.datasets_path + '//' + args.datasets_train
    tg_folder = args.datasets_path + '//' + args.datasets_target
    ...
    for im_name in list(os.walk(im_folder, topdown=False))[-1][-1]:
        print(im_name)
        im_dir = im_folder + '//' + im_name
        tg_dir = tg_folder + '//' + im_name

        noise_im = tiff.imread(im_dir)
        target_im = tiff.imread(tg_dir)
        ...
        # print('noise_im min -----> ',noise_im.min())
        noise_im = (noise_im-noise_im.min()).astype(np.float32)/args.normalize_factor
        target_im = (target_im-target_im.min()).astype(np.float32)/args.normalize_factor
        ...
        for x in range(0,int((whole_h-img_h+gap_h)/gap_h)):
            for y in range(0,int((whole_w-img_w+gap_w)/gap_w)):
                ...
                single_coordinate['end_w'] = end_w
                # noise_patch1 = noise_im[init_s:end_s,init_h:end_h,init_w:end_w]
                patch_name = im_name.replace('.tif','')+'_x'+str(x)+'_y'+str(y)            
                # train_raw.append(noise_patch1.transpose(1,2,0))
                name_list.append(patch_name)
                # print(' single_coordinate -----> ',single_coordinate)
                coordinate_list[patch_name] = single_coordinate
    return  name_list, noise_im, target_im, coordinate_list
...
def test_preprocess_lessMemoryNoTail_chooseOne (args, N):   #测试时的图像预处理
    img_h = args.img_h
    img_w = args.img_w
    gap_h = args.gap_h
    gap_w = args.gap_w
    ...
    for x in range(0,num_h):
        for y in range(0,num_w):
                ...
                # noise_patch1 = noise_im[init_s:end_s,init_h:end_h,init_w:end_w]
                patch_name = args.datasets_train+'_x'+str(x)+'_y'+str(y)
                # train_raw.append(noise_patch1.transpose(1,2,0))
                name_list.append(patch_name)
                # print(' single_coordinate -----> ',single_coordinate)
                coordinate_list[patch_name] = single_coordinate
